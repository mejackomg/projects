...
import {loadOrderRevoke,loadOrderRevoke1} from '../../../reducers/database.js';
import OrderRevokeTable from './OrderRevokeTable.jsx'

class OrderRevokeDetail extends Component {
    constructor(props) {
        super(props);
    ...
    getOption() {
        const {OrderRevokeData} = this.props;
        var option = null;
        if (OrderRevokeData.length > 0) {
            var xData = [];
            var y1Data = [];
            var y2Data = [];
            var y3Data = [];
            for (var i = 0; i < OrderRevokeData.length; i++) {
                xData.push(OrderRevokeData[i]['日期']);
                y1Data.push(OrderRevokeData[i]['销售额']);
                y2Data.push(OrderRevokeData[i]['外卖次数']);
                y3Data.push(OrderRevokeData[i]['到店次数']);
            }
    ...
    componentDidMount() {
        ...
        dispatch(loadOrderRevoke(shopsId, radioIndex));
    }
    ...
    handleChangeRadio = (event, value) => {
        ...
        dispatch(loadOrderRevoke(shopsId, value));
    };
    handleChangeFromDate = (event, date) => {
        ...
        if (date && toDate) {
            dispatch(loadOrderRevoke1(shopsId, date, toDate));
        }
    };
    ...
    handleChangeToDate = (event, date) => {
        ...
        if (fromDate && date) {
            dispatch(loadOrderRevoke1(shopsId, fromDate, date));
        }
    };
    ...
    render() {
        ...
        const {shopsId, shops_name, OrderRevokeData} = this.props;
        const {page, pageLineCount}=this.state;
        const pageConut = Math.ceil(OrderRevokeData.length / pageLineCount);
        return (
            <div>
                <div
                    style={{display: 'flex', height: '80px', marginTop: '20px', padding: '0 30px'}}>
                ...
                <div >
                    <OrderRevokeTable
                        ...
                        OrderRevokeData={OrderRevokeData}
                        shops_Id={shopsId}
...
OrderRevokeDetail.propTypes = {
    dispatch: PropTypes.func.isRequired,
};

OrderRevokeDetail.contextTypes = {
...
const mapStateToProps = (state) => {
    ...
    return {
        OrderRevokeData: databaseReducer.OrderRevokeData
    };
};
